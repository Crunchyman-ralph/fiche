...
function isWhitespaceOnly(del, add) {
  return (
    del.replace(/^-/, "").replace(/\s/g, "") ===
    add.replace(/^\+/, "").replace(/\s/g, "")
  );
}
...
function detectMoves(dels, adds) {
  ...
  for (var di = 0; di < dels.length; di++) {
    ...
    var dn = db.map(function (i) {
      return normWs(dels[i].code);
    });
    for (var ai = 0; ai < adds.length; ai++) {
      ...
      var an = ab.map(function (i) {
        return normWs(adds[i].code);
      });
      var ml = Math.min(dn.length, an.length),
        mc = 0;
      for (var m = 0; m < ml; m++) {
        if (dn[m] === an[m]) mc++;
      }
...
function renderDiff(target, diffInput) {
  ...
  if (!lines.length) {
    el.innerHTML =
      '<div style="padding:12px;color:#777;font-size:12px;">No diff data</div>';
    return;
  }
  ...
  for (var ri = 0; ri < parsed.length; ri++) {
    ...
    if (p.type === "hunk") {
      ...
      var ai2 = -1;
      for (var fa = 0; fa < adds.length; fa++)
        if (adds[fa].idx === p.idx) {
          ai2 = fa;
          break;
        }
      var cls = mv.movedAdds[ai2]
        ? mv.movedAdds[ai2].exact
          ? "diff-moved-add"
          : "diff-moved-add-edited"
        : "diff-add";
      rows.push(
        '<tr class="' +
          cls +
          '"><td class="diff-ln"></td><td class="diff-ln">' +
          p.newLine +
      ...
      var di2 = -1;
      for (var fd = 0; fd < dels.length; fd++)
        if (dels[fd].idx === p.idx) {
          di2 = fd;
          break;
        }
      var cls2 = mv.movedDels[di2]
        ? mv.movedDels[di2].exact
          ? "diff-moved-del"
          : "diff-moved-del-edited"
        : "diff-del";
      rows.push(
        '<tr class="' +
          cls2 +
      ...
      rows.push(
        '<tr class="diff-ctx"><td class="diff-ln">' +
          p.oldLine +
          '</td><td class="diff-ln">' +
          p.newLine +
          '</td><td class="diff-code">' +
